...
mod tests {
    ...
    fn test_seq_input_fastq_output_fastq() {
        ...
        // 02 - mask 
        let args: Vec<&str> = "seq -I tests/data/chr.fastq --drop-ambigous-seq --mask-char x --lowercases-to-char".split_whitespace().collect();
        let output = run_program_with_args(&args);
        ...
        let args: Vec<&str> = "seq -I tests/data/chr.fastq --q-high 20 --uppercases --line-len 20 --trim-header".split_whitespace().collect();
        let output = run_program_with_args(&args);
        ...
        // 06 - ascii bases
        let args: Vec<&str> = "seq -I tests/data/long.fastq -l 1000 --ascii-bases 35 --output-qual-33".split_whitespace().collect();
        let output = run_program_with_args(&args);
    ...
    fn test_seq_input_fastq_output_fasta() {
        ...
        // 06 - ascii bases
        let args: Vec<&str> = "seq -I tests/data/long.fastq -l 1000 --ascii-bases 35 --q-low 5 --output-fasta".split_whitespace().collect();
        let output = run_program_with_args(&args);
    ...
    fn test_seq_input_fasta_output_fasta() {
        ...
        // 04 - 
        let args: Vec<&str> = "seq -A tests/data/chr.fasta --uppercases --line-len 20 --trim-header --output-fasta".split_whitespace().collect();
        let output = run_program_with_args(&args);
    ...
    fn test_seq_input_fasta_output_fastq() {
        // 01 - output fastq
        let args: Vec<&str> = "seq -A tests/data/chr.fasta"
            .split_whitespace()
            .collect();
        let output = run_program_with_args(&args);
        let expect_content = fs::read("tests/data/chr.fasta").expect("");
        assert_eq!(output.as_bytes(), expect_content, "[test] fa -> fq - 01");
        // 02 - 
        let args: Vec<&str> = "seq -A tests/data/chr.fasta --drop-ambigous-seq --mask-char x --lowercases-to-char".split_whitespace().collect();
        let output = run_program_with_args(&args);
        let expect_content = fs::read("tests/data/seq/fq2fa_02.fasta").expect("");
        ...
        // 04 - 
        let args: Vec<&str> = "seq -A tests/data/chr.fasta --uppercases --line-len 20 --trim-header".split_whitespace().collect();
        let output = run_program_with_args(&args);
        let expect_content = fs::read("tests/data/seq/fa2fa_04.fasta").expect("");
